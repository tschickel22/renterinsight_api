class InstallSolidQueueInPrimaryDb < ActiveRecord::Migration[8.0]
  def change
    ...
    %i[blocked_executions claimed_executions failed_executions ready_executions
       recurring_executions scheduled_executions].each do |t|
      next if foreign_key_exists?("solid_queue_#{t}", :solid_queue_jobs, column: :job_id)
      add_foreign_key "solid_queue_#{t}", :solid_queue_jobs, column: :job_id, on_delete: :cascade
    end
  end
end
