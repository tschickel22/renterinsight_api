class AddInventoryCostDetailsResource < ActiveRecord::Migration[8.0]
  def up
    # Create new resource for inventory cost details
    inventory_cost_resource = Resource.create!(
      key: 'inventory_cost_details',
      name: 'Inventory (Cost Details)',
      description: 'View and edit dealer cost, margins, and pricing details for inventory items',
      category: 'operations',
      permission_groups: {
        'view_edit_costs' => {
          'name' => 'View & Edit Cost Details',
          'description' => 'View dealer cost, freight, PDI, margins, and edit pricing fields',
          'group' => 'financial',
          'actions' => ['read', 'update']
        }
      }
      ...
      when 'platform'
        ...
        [read_action, update_action, create_action, delete_action, export_action].each do |action|
          RolePermission.find_or_create_by!(
            role: role,
            resource: inventory_cost_resource,
            action: action,
    ...
    puts "✅ Created 'Inventory (Cost Details)' resource"
    puts "✅ Granted permissions to: #{roles_with_access.map(&:name).join(', ')}"
  end

  def down
    # Remove the resource (will cascade delete role_permissions)
    resource = Resource.find_by(key: 'inventory_cost_details')
    resource&.destroy
    
    puts "🗑️  Removed 'Inventory (Cost Details)' resource"
  end
end
