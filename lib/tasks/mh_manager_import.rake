...
# =============================================================================
# MH Manager — generic onboarding import loader
# =============================================================================
# Idempotent, section-by-section import of an MH Manager export into a company.
# Reads normalized CSVs from IMPORT_CSV. Column names verified against schema.rb.
#
# This is the GENERIC, env-driven successor to the original one-off Evangeline
# loader. Nothing here is customer-specific: the company name, subdomain,
# locations, batch stamp, ticket-dedup prefix, and CSV path all come from env
# vars (or, for locations, a locations.csv in the CSV dir). Run it for any new
# MH Manager customer by setting the env vars below.
#
# ---------------------------------------------------------------------------
# REQUIRED CONFIG (env)
#   IMPORT_COMPANY_NAME="Acme Home Center"   display name (bootstrap create)
#   IMPORT_SUBDOMAIN="acme-homes"            unique subdomain (lookup + create)
#   IMPORT_CSV=/data/acme                    path to the normalized CSV folder
#
# OPTIONAL CONFIG (env)
#   IMPORT_COMPANY_ID=NN     pin to an EXISTING company (skips subdomain lookup;
#                            use this for every step after bootstrap when you
#                            want to be certain you're writing to the right co)
#   IMPORT_BATCH=acme_2026_06   batch stamp written to custom_field_values
#                               (default: "<subdomain>_<YYYY_MM>")
#   IMPORT_TICKET_PREFIX=ACME   service-ticket dedup prefix (default: derived
#                               from subdomain, uppercased, non-alnum stripped)
#   DRY_RUN=1                parse + report, write nothing
#
# ---------------------------------------------------------------------------
# CSV FILES EXPECTED in IMPORT_CSV (each optional — a missing file is skipped):
#   locations.csv      code,name,address,city,state,zip,phone
#   sources.csv        name
#   users.csv          email,first_name,last_name,role_key,access_tier,location_scope
#   manufacturers.csv  name,factory_code,active,service_mgr_email,service_mgr_phone,service_mgr_name
#   contractors.csv    name,address,city,state,zip,phone,email,email2,phone2,gl_insurer,gl_expiration,wc_insurer,wc_expiration
#   inventory.csv      serial_number,stock_number,status,year,make,model,bedrooms,bathrooms,msrp,dealer_cost,condition,wind_zone,notes,location_name,virtual_tour_url,building_code,hud_number,date_acquired,source_model_id
#   accounts.csv       name/household_flipped,email,location_name,source,salesperson,billing_*,file_number,finance_method,date_sold,serial_number,series,model,model_year,size,building_code,hud_number,county,notes
#   contacts.csv       first_name,last_name,email,phone,is_primary,account_name,location_name
#   leads.csv          first_name,last_name/name,email,phone,status,source,salesperson,location_name,notes,link_inventory_serial,home_interest,rating,ordered_model
#   deals.csv          name,contact_first,contact_last,account_name,owner,email,phone,location_name,source_name,stage,vehicle_serial,vehicle_stock,down_payment,close_reason,file_number,default_surcharge,notes
#   service_tickets.csv source_id,title,description,ticket_status,priority,account_name,vehicle_serial,factory_name,claim_status,is_warranty,location_name,warranty_start_date,building_code,hud_number,notes_internal
#
# ---------------------------------------------------------------------------
# RUN ORDER (each idempotent — safe to re-run):
#   bin/rails mh_import:bootstrap        # company + locations(csv) + 21st lender
#   bin/rails mh_import:custom_fields    # per-module custom fields (UI-visible)
#   bin/rails mh_import:sources
#   bin/rails mh_import:users
#   bin/rails mh_import:manufacturers
#   bin/rails mh_import:contractors
#   bin/rails mh_import:inventory
#   bin/rails mh_import:accounts
#   bin/rails mh_import:contacts
#   bin/rails mh_import:leads
#   bin/rails mh_import:deals
#   bin/rails mh_import:service_tickets
#   bin/rails mh_import:reconcile        # deferred-link sweep
#   bin/rails mh_import:all              # all of the above, in order
#   bin/rails mh_import:status           # batch counts
#   bin/rails mh_import:rollback CONFIRM=yes   # delete the test company (cascade)
# =============================================================================

require 'csv'

namespace :mh_import do
  # ---- config (env-driven) --------------------------------------------------
  def cfg_company_name = ENV['IMPORT_COMPANY_NAME'].to_s.strip
  def cfg_subdomain    = ENV['IMPORT_SUBDOMAIN'].to_s.strip
  def cfg_csv_dir      = ENV.fetch('IMPORT_CSV', '/data/import')
  def cfg_dry          = ENV['DRY_RUN'].present?

  def cfg_batch
    return ENV['IMPORT_BATCH'].strip if ENV['IMPORT_BATCH'].present?
    base = cfg_subdomain.presence || 'import'
    "#{base.gsub(/[^a-z0-9]+/i, '_')}_#{Time.current.strftime('%Y_%m')}"
  end

  # Service-ticket dedup prefix. Derived from the subdomain unless overridden:
  #   "acme-homes" -> "ACMEHOMES". Kept stable + unique per customer so re-runs
  #   match prior rows on ticket_number and skip them.
  def cfg_ticket_prefix
    return ENV['IMPORT_TICKET_PREFIX'].strip if ENV['IMPORT_TICKET_PREFIX'].present?
    (cfg_subdomain.presence || 'IMP').gsub(/[^a-z0-9]+/i, '').upcase
  end

  # Convenience accessors used throughout the tasks.
  def batch    = (@__batch ||= cfg_batch)
  def csv_dir  = (@__csv_dir ||= cfg_csv_dir)
  def dry?     = cfg_dry
  ...
  # Resolve the target company. IMPORT_COMPANY_ID pins to an existing company
  # (safest for post-bootstrap steps); otherwise look up by IMPORT_SUBDOMAIN.
  def import_company
    @import_company ||= if ENV['IMPORT_COMPANY_ID'].present?
      Company.find(ENV['IMPORT_COMPANY_ID'])
    else
      Company.find_by(subdomain: cfg_subdomain)
    end
  end

  def require_company!
    if cfg_subdomain.blank? && ENV['IMPORT_COMPANY_ID'].blank?
      abort('Set IMPORT_SUBDOMAIN (and IMPORT_COMPANY_NAME) or IMPORT_COMPANY_ID first.')
    end
    import_company || abort(
      "No company found for subdomain='#{cfg_subdomain}'. " \
      'Run `mh_import:bootstrap` first (or pass IMPORT_COMPANY_ID=NN).'
    )
  end
  ...
  #
  # Price/cost mapping (verified against MH Manager data + the loaded Vehicle):
  ...
  # BOOTSTRAP
  # ===========================================================================
  desc 'Create company + locations (from locations.csv) + 21st Mortgage lender'
  task bootstrap: :environment do
    log("== #{cfg_company_name.presence || cfg_subdomain} bootstrap ==#{dry_note}")

    if cfg_subdomain.blank?
      abort('IMPORT_SUBDOMAIN is required for bootstrap (and IMPORT_COMPANY_NAME for a fresh create).')
    end

    company = Company.find_by(subdomain: cfg_subdomain)
    if company
      log("  Company exists: #{company.name} (id=#{company.id})")
    elsif dry?
      log("  Would create company '#{cfg_company_name}' subdomain=#{cfg_subdomain} industry=manufactured_housing")
    else
      abort('IMPORT_COMPANY_NAME is required to create a new company.') if cfg_company_name.blank?
      company = Company.create!(
        name: cfg_company_name, subdomain: cfg_subdomain,
    ...
    if company && !dry?
      ...
      if locations.empty?
        log('  No locations.csv — leaving the auto-created default location in place.')
      else
        main = company.locations.where('name ILIKE ?', 'Main Location').first
        locations.each_with_index do |loc, idx|
          code = col(loc, 'code').to_s.strip
          name = col(loc, 'name').to_s.strip
          next if name.blank?
          existing = company.locations.where('code = ? OR name ILIKE ?', code, name).first
          target   = existing || (idx.zero? ? main : nil)
          attrs = {
            name: name, code: code.presence,
            address_line1: col(loc, 'address', 'address1', 'address_line1'),
            city: col(loc, 'city'), state: col(loc, 'state'),
            zip_code: col(loc, 'zip', 'zip_code', 'postal_code'),
            phone: col(loc, 'phone'), active: true,
            timezone: col(loc, 'timezone').presence || 'America/Chicago'
          }
          if target
            target.update!(attrs)
            log("  ✅ Location ready: #{name} (#{code}) id=#{target.id}")
          else
            created = company.locations.create!(attrs)
            log("  ✅ Location created: #{name} (#{code}) id=#{created.id}")
          end
        end
      end
  ...
  task manufacturers: :environment do
    ...
    read_csv('manufacturers').each do |row|
      ...
      mfr = Manufacturer.create!(
        ...
        active: row['active'] != 'false', industry_type: 'manufactured_home',
  ...
  task leads: :environment do
    ...
    sample_errors = []
    now = Time.current
    pending = []   # plain attribute hashes for bulk insert
  ...
  task deals: :environment do
    ...
    read_csv('deals').each do |row|
      ...
      # The model requires an account OR a contact. Active-deal buyers may not be
      # in the accounts/contacts import (those are the sold-customer sets), so
      # many rows resolve nothing. Create the household account + primary contact
      ...
      # Map source stage -> valid pipeline stage. Active 'Working Deal'/'working'
      # rows -> 'negotiation'.
  ...
  task service_tickets: :environment do
    ...
    read_csv('service_tickets').each do |row|
      ...
      ticket_no = "#{cfg_ticket_prefix}-#{col(row, 'source_id', 'source_ticket_id')}"   # stable, unique, indexed dedup key
  ...
  #
  # SAFETY: refuses unless the resolved company's subdomain matches
  # IMPORT_SUBDOMAIN exactly AND CONFIRM=yes. This prevents a stray
  # IMPORT_COMPANY_ID from nuking the wrong company.
  # ===========================================================================
  desc 'Delete the import TEST company and ALL its data (FK-safe)'
  task rollback: :environment do
    company = require_company!
    if cfg_subdomain.blank? || company.subdomain != cfg_subdomain
      abort("Refusing: company #{company.id} (subdomain=#{company.subdomain.inspect}) " \
            "does not match IMPORT_SUBDOMAIN=#{cfg_subdomain.inspect}. " \
            'Set IMPORT_SUBDOMAIN to the exact subdomain to confirm the target.')
    end
