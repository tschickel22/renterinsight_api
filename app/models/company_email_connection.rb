...
# == Schema Information
#
# Table name: company_email_connections
#
# Same shape as user_email_connections, scoped per company.
# One connection per company (enforced by unique index on company_id).

class CompanyEmailConnection < ApplicationRecord
  belongs_to :company
  ...
  def refresh_oauth_token!(scope: nil, flag_dead_grant: true)
    ...
    Rails.logger.error "[CompanyEmailConnection#refresh_oauth_token!] Failed: #{e.message}"
    nil
  end
  ...
  def note_dead_grant!(message)
    ...
    # Campaigns sending as this company pause now instead of failing recipients.
