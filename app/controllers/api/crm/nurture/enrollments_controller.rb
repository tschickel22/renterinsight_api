class Api::Crm::Nurture::EnrollmentsController < ApplicationController
  # JSON SPA calls won't send CSRF tokens (handle both API-only and full Rails)
  ...
  def index
    ...
    render json: scope.as_json(only: %i[
      id lead_id nurture_sequence_id status current_step_index created_at updated_at
    ])
  rescue => e
  ...
  # Body:
  # { upsert:[{ id?, lead_id?, nurture_sequence_id?, status?, current_step_index? }], delete:[ids] }
  # Also accepts the same payload nested under { enrollment: { ... } }.
  def bulk
    payload = bulk_params
    ...
    ActiveRecord::Base.transaction do
      upserts.each do |attrs|
        ...
        # Normalize legacy status from UI, just in case.
        attrs[:status] = 'running' if attrs[:status] == 'active'

        rec =
          if attrs[:id].present?
            NurtureEnrollment.find(attrs[:id])
          elsif attrs[:lead_id].present? && attrs[:nurture_sequence_id].present?
            NurtureEnrollment.where(
              lead_id: attrs[:lead_id],
              nurture_sequence_id: attrs[:nurture_sequence_id]
            ).where.not(status: 'completed').order(:id).first
          end

        if rec
          # Only fields that can change after creation
          rec.update!(
