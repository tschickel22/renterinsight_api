...
module Api
  module Crm
    class LeadActivitiesController < ApplicationController
      ...
      def create
        Rails.logger.info "[LeadActivitiesController#create] Starting create with params: #{params.inspect}"
        
        @activity = @lead.lead_activities.build(activity_params)
        @activity.user = current_user_or_first
        @activity.assigned_to ||= current_user_or_first
        
        Rails.logger.info "[LeadActivitiesController#create] Built activity: #{@activity.attributes.inspect}"

        if @activity.save
          Rails.logger.info "[LeadActivitiesController#create] Activity saved successfully with id=#{@activity.id}"
          render json: activity_json(@activity), status: :created
        else
          Rails.logger.error "[LeadActivitiesController#create] Validation failed: #{@activity.errors.full_messages}"
          render json: { errors: @activity.errors.full_messages }, status: :unprocessable_entity
        end
      rescue => e
        Rails.logger.error "[LeadActivitiesController#create] #{e.class}: #{e.message}"
      ...
      def update
        ...
        Rails.logger.error "[LeadActivitiesController#update] #{e.class}: #{e.message}"
      ...
      def complete
        ...
        Rails.logger.error "[LeadActivitiesController#complete] #{e.class}: #{e.message}"
      ...
      def cancel
        ...
        Rails.logger.error "[LeadActivitiesController#cancel] #{e.class}: #{e.message}"
