class Api::V1::Admin::ManufacturersController < ApplicationController
  ...
  # PATCH /api/v1/admin/manufacturers/:id
  def update
  ...
  # DELETE /api/v1/admin/manufacturers/:id (soft delete via deactivate)
  def destroy
  ...
  # POST /api/v1/admin/manufacturers/:id/activate
  def activate
  ...
  # POST /api/v1/admin/manufacturers/:id/deactivate
  def deactivate
    if @manufacturer.deactivate!
      Rails.logger.info("[Admin] Platform admin #{current_user.email} deactivated manufacturer: #{@manufacturer.name}")
      render json: serialize_manufacturer(@manufacturer)
