class Api::V1::CampaignSuppressionsController < ApplicationController
  ...
  def index
    ...
    if params[:search].present?
      suppressions = suppressions.where('email_address ILIKE ?', "%#{params[:search].downcase}%")
    end
    ...
    render json: {
      items: suppressions.map { |s| { id: s.id, email_address: s.email_address, reason: s.reason, suppressed_at: s.suppressed_at, notes: s.notes } },
  ...
  def create
    ...
    if s.save
      render json: { id: s.id, email_address: s.email_address, reason: s.reason }, status: :created
    else
